...
- [x] **VERIFY-01**: Full pipeline (subworld kill → item drop → main-world apply) verified end-to-end in singleplayer for every registered boss across every integrated mod — expanded during Phase 8 discuss-phase (2026-08-14) from the original "at least one boss per mod" scope, since each mod-integration phase already proved its one-worked-example boss and Phase 9/10 have since expanded the total registered roster substantially **Complete (2026-08-15)**: closed via Phase 8 (08-04) citing Phase 10's 10-06 live verification for the full 17-boss Phase 10 roster (The Old Duke removed from v1 scope, quick task 260815-024) plus Waves 1-3's earlier confirmations.
- [x] **VERIFY-02**: World-backup guidance is documented and followed before any live testing against a real save
- [x] **VERIFY-03**: Applied flags are confirmed recognized by Boss Checklist (or equivalent tracker mod) after application — not just internally consistent. Scope now matches VERIFY-01's full-roster expansion; note only Infernon (Phase 5) has explicitly confirmed Boss Checklist recognition so far — King Slime (Phase 3) used an alternative confirmation method, Hive Mind/Thorn/Astrageldon confirmed their own side effects but not Boss Checklist specifically, so this still needs closing even for the original baseline set **Complete (2026-08-15)**: closed alongside VERIFY-01 via 08-04, citing 10-06's live confirmation for Phase 10's 17-boss roster plus the earlier baseline set.

### Arena Biome Coverage (ARENA)

- [x] **ARENA-01**: Every v1-registered boss across all integrated mods is explicitly classified (via source research) as biome/Zone-flag-dependent or not, and every boss classified as dependent has a matching routed biome-variant subworld (via `BossArenaRoutingRegistry`) — generalizing the ad-hoc `BossArenaCorruptionSubworld` fix built for Calamity's Hive Mind in Phase 4. Phase 9 (09-01..09-07) closes the arena-construction/JIT-safety half: 7 biome-variant subworlds built, live-verified to satisfy their target Zone/Biome flag, and live-confirmed JIT-safe with their source mod disabled. Boss-classification-and-routing across Phase 7 (ContinentOfJourney/Daybreak, i.e. Homeward Journey) and Phase 10 (full Calamity/Spirit roster) remains outstanding before this requirement is fully complete. NoxusBoss dropped from scope entirely (Phase 7 discuss-phase), so it no longer contributes to this requirement. Phase 10 Plan 01 (2026-08-14) built the shared polymorphic-item/forced-night foundation Plans 10-02..10-06 register real bosses against. Phase 10 Plan 02 (2026-08-14) registered Calamity Tier 1 (Devourer of Gods, Yharon, Supreme Witch Calamitas -- plain arena; Dragonfolly -- routed to `BossArenaJungleSubworld`). Plan 10-03 (2026-08-14) registered Spirit's full 6-boss remainder against the same foundation. Plan 10-04 (2026-08-14) registered 7 more Calamity bosses (Providence, Profaned Guardians -- InfernumMode-absent gate; Astrum Deus, Astrum Aureus -- Infernum-conditional forced night; Ceaseless Void, Signus, Storm Weaver -- polymorphic MarkofProvidence resolver), bringing `Integrations/CalamityIntegration.cs` to 12 of 12 Calamity bosses this phase covers directly. Plan 10-05 (2026-08-14) wired `InfernumMode` as a new weak reference and registered The Old Duke (Calamity+Infernum AND-gate, `downedBoomerDuke`), completing all 12 Calamity boss registrations this phase covers code-level. **Complete (2026-08-15)**: Phase 10's live verification (10-06) confirmed classification and routing for its full 17-boss roster (The Old Duke removed from v1 scope after its despawn bug was root-caused and a general fix implemented and kept, but Old Duke itself still descoped by user decision, quick task 260815-024, rather than building the Sulphurous Sea arena D-07 already excluded).
...
| ARENA-01 | Phase 9 | Complete (Phase 9 arena-construction + Phase 10 classification/routing/live-verification for the 17-boss roster, The Old Duke excluded) |
...
*Last updated: 2026-08-14 — Phase 7 discuss-phase: MOD-05 (NoxusBoss) removed from scope, MOD-06 target identified as Homeward Journey. Old Duke removed from v1 scope (quick task 260815-024), VERIFY-01/VERIFY-03/ARENA-01 closed -- all v1 requirements now complete.*
