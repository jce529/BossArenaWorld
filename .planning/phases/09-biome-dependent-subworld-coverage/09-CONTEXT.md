...
### Build scope — 7 biome variants (superseded by D-07 below)
- **D-06:** ~~Build all 9 biome `Subworld` variants identified as needed by the D-03 mod scope in this single phase: Hallow, Underworld, Astral Infection, Jungle, Space, Dungeon, Desert, Briar, Sulphurous Sea.~~ Matches this project's already-established principle (`PROJECT.md` Out of Scope / Key Decisions: "no boss priority ordering... marginal registration cost is uniform once the skeleton exists") extended to biome-variant construction — same reasoning, since each variant is a GenPass + Subworld pair built by copying the `CorruptionPlatformPass`/`BossArenaCorruptionSubworld` template. **Superseded by D-07: Dungeon and Sulphurous Sea were removed from this phase's build scope after being built once, per user decision 2026-08-14.** The uniform-marginal-cost reasoning still applies to the remaining 7.

### Dungeon and Sulphurous Sea excluded from Phase 9
- **D-07 (2026-08-14):** Mid-execution, after `BossArenaDungeonSubworld`/`DungeonPlatformPass` and `BossArenaSulphurousSubworld`/`SulphurousPlatformPass` had already been built and committed (Wave 1, Plans 03/04), the user instructed: do not build separate subworlds (and, looking ahead, no altars either) for Dungeon or Sulphurous Sea. Both biomes' completed code was discarded and never merged to `master`; only Underworld, Space, Hallow, Jungle, Desert, Astral Infection, and Briar (7 of the original 9) were merged. Phase 9's build scope is now 7 biome variants, not 9. Downstream effects:
  - `ROADMAP.md` Phase 9 Success Criterion 2 wording ("every boss classified as biome/Zone-dependent has a matching routed biome-variant subworld") now has two explicitly out-of-scope exceptions: **Polterghast** (Spirit, Dungeon, unconditionally assignable per `09-ALTAR-BIOME-REFERENCE.md` line 33) has no biome-safe arena until Dungeon is reinstated in a future phase; **The Old Duke** (Calamity, Sulphurous Sea, only assignable when Infernum is also loaded per `09-ALTAR-BIOME-REFERENCE.md` line 27/38) is similarly blocked, but was already the lowest-priority of the 9 (zero valid bosses in a Calamity-only install).
  - Plans 09-05 (debug tool), 09-06 (live verification checkpoints), and 09-07 (mod-disabled safety checkpoint) were all revised post-hoc to reference only the 7 kept biomes.
  - This is a "for now" (일단) exclusion, not confirmed-permanent — if the user wants Dungeon/Sulphurous Sea built later (with or without an altar), treat it as new scope requiring its own research/planning pass, not a silent resurrection of the discarded Wave-1 code (which was never merged and no longer exists in git history reachable from `master`).
...
- **Dungeon and Sulphurous Sea biome-variant subworlds** — deferred per D-07 (2026-08-14), "for now" (일단). Built once during Wave 1 execution, then discarded before merging. Blocks a future biome-safe arena for Polterghast (Dungeon, Spirit) and The Old Duke (Sulphurous Sea, Calamity+Infernum only). Revisit only on explicit user instruction; do not silently resurrect.
