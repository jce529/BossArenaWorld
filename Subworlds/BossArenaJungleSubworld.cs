...
namespace BossArenaSubWorld.Subworlds
{
	...
	// platform is Jungle-biome-flavored (JungleGrass, via JunglePlatformPass) instead of plain Stone
	// or Corruption.
	//
	// WHY THIS EXISTS (see 09-RESEARCH.md): decompiled Terraria.Player.UpdateBiomes() sets
	// `player.ZoneJungle = SceneMetrics.JungleTileCount >= SceneMetrics.JungleTileThreshold` (140), a
	// per-tick weighted sum over TileID.Sets.JungleBiome -- the same tile-weighted mechanism family
	// as Corruption's ZoneCorrupt. CRITICAL: Mud (the vanilla Jungle "body" tile) carries ZERO weight
	// in that table, so JunglePlatformPass fills the FULL platform thickness with JungleGrass rather
	// than a thin surface veneer (09-RESEARCH.md Pitfall 2). Biome-gated bosses that require
	// player.ZoneJungle (e.g. Calamity's Dragonfolly, routed here in a later phase via
	...
	public class BossArenaJungleSubworld : Subworld
	{
